...
THEME_RULES = [
    ("平台活动运营审计", ["补贴", "佣金", "核销", "复盘", "证据链", "处罚", "退款率", "低质", "异常", "风控", "审计", "投入产出", "线索清单", "数据分散"]),
...
THEME_KEYWORD_BANK = {
    "平台活动运营审计": ["补贴", "佣金", "核销", "复盘", "数据分散", "证据链", "投入产出", "线索清单", "异常交易", "退款率"],
...
def analyze_interview_rule_based(text: str) -> dict:
    ...
    key_points = []
    # 审计场景的关键要点
    if "平台活动运营审计" in final_theme:
        if "数据分散" in all_keywords or "多系统" in all_keywords:
            key_points.append("数据分散在多个系统，跨表核对耗时，需要整合形成统一视图。")
        if "补贴" in all_keywords or "佣金" in all_keywords:
            key_points.append("补贴和佣金规则复杂，执行过程中不断补充调整。")
        if "证据链" in all_keywords or "线索清单" in all_keywords:
            key_points.append("审计关注活动全链条证据，需要形成线索清单。")
    # 通用场景
    elif "提示不清" in all_keywords or "流程复杂" in all_keywords:
        key_points.append("受访者主要困难集中在流程复杂和系统提示不清。")
    elif "线下兜底" in all_keywords or "人工辅导" in all_keywords:
...
def extract_local_features(raw: str) -> dict:
    ...
    # 受访者类型细分识别（根据具体岗位）
    if any(w in raw for w in ["运营负责人", "主管", "经理"]):
        respondent_type = "基层管理"
    elif any(w in raw for w in ["专员", "分析师", "产品经理", "财务", "风控", "客服"]):
        respondent_type = "一线执行"
    elif any(w in raw for w in ["运营", "网格员", "网格", "窗口", "工作人员", "信息科", "民警", "护士", "老师", "医生", "保安", "物业", "保洁", "居委"]):
        respondent_type = "一线执行"
    ...
    for pattern in age_patterns:
        age_match = re.search(pattern, raw)
        if age_match:
            break
    ...
    # 情绪识别 - 修复"无奈"优先级
    anxiety_words = ["担心", "焦虑", "紧张", "害怕", "失败", "排队", "被骗", "挫败", "拖到", "压力", "麻烦", "难受", "尴尬", "不满", "气愤", "委屈", "崩溃", "头疼"]
    helpless_words = ["无奈", "没办法", "只能", "还得", "太难了", "不知道怎么办"]
    positive_words = ["方便", "提升", "省事", "顺畅", "高效", "认可", "感谢", "满意", "感谢", "挺好", "不错"]

    if any(w in raw for w in helpless_words):
        emotion = "无奈"
    elif any(w in raw for w in anxiety_words):
        emotion = "焦虑"
...
要求：
1. 根据文本内容，准确识别以下字段：
   - "受访者自述身份"：直接提取受访者在文本中明确说出的具体岗位/身份（如"活动运营负责人"、"用户增长运营"、"主播运营专员"、"内容治理运营"、"平台数据分析师"、"财务运营"、"活动系统产品经理"、"客服运营"、"风控运营"、"内部审计主管"），不要"我是"、"我是一名"等前缀
   - "受访者类型"：根据角色层级判断（一线执行/基层管理/中层管理/高层管理/普通用户），不是简单的工作人员分类
   - "具体年龄"：如果文本明确提到年龄（如"我今年34岁"），提取具体数字；否则设为空
   - "年龄段"：根据具体年龄判断：18岁以下返回"未成年"，18-25岁返回"青年"，26-35岁返回"中青年"，36-45岁返回"中年"，46-60岁返回"中老年"，60岁以上返回"高龄"；如果无法确定具体年龄，则返回"未知"
   - "核心主题"：根据文本内容从以下主题中选择最符合的（平台活动运营审计/用户补贴效果评估/主播佣金管理/内容治理与处罚/活动数据复盘/财务核算与成本控制/系统产品功能/客服与申诉处理/风控与异常识别/审计线索分析/数字医疗服务/政务数字化服务/教育数字平台/反诈与数字安全/数字平台经营/日常数字生活），**必须根据每条访谈的具体内容差异化判断，不要所有访谈都归为同一主题**
...
"""
    ...
    try:
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "system", "content": "你是中文访谈分析助手，擅长从访谈文本中提取精准、差异化的事实信息。请严格按照每条访谈的具体内容分析，不要泛化或统一归类。"},
